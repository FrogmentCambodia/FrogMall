//
//  SearchMoreViewController.swift
//  FrogMallFir
//
...
class SearchMoreViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    let sectionUP2 = ["Category"]
    let sectionUP3 = ["Area"]
...
*/
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.performSegue(withIdentifier: "ToSearchCollect", sender: nil)
    }
    
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any!) {
        if (segue.identifier == "ToSearchCollect") {
            let SCVC: SearchCollectViewController = (segue.destination as? SearchCollectViewController)!
            
            if getCell == "Category" {
                SCVC.getCate = setCell2
/*
                SCVC.showIndicator()
                SCVC.SearchCollect.reloadData()
                SCVC.indicator.stopAnimating()
 */
            } else {
